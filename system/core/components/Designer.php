...
class VoidDesigner__unstable extends Component
{
    protected $form;
    protected $control;
    protected $host;
    ...
    public function __construct (Control $parent, string $formName = 'form', PropertyGrid $propertyGrid, ListBox $eventsList, ComboBox $currentSelectedItem, TabControl $formsList)
    {
        ...
        $this->componentSelector = VoidEngine::createObject (new WFObject ('WinForms_PHP.FormDesigner3', false, true), $this->form->selector, $formName);
        Components::addComponent ($this->componentSelector, $this);

        $this->control = $this->callMethod ('GetControl');
        $this->host    = $this->callMethod ('GetHost');
        ...
        VoidEngine::setObjectEvent ($this->control, 'KeyDown', '
            ...
            if ($args->keycode == 46)
                _c('. $this->componentSelector .')->removeSelected ();
        ');
        ...
        for ($i = 0; $i < $size; ++$i)
        {
            ...
            else
            {
                if ($this->formsList->items->count > 1)
                {
                    ...
                    $designer = VoidStudioAPI::getObjects ('main')['Designer__'. _c($this->formsList->selectedTab)->text .'Designer'];
